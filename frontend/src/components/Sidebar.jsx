...
export const Sidebar = () => {
  const { user } = useAuth();
  const locked = user?.onboardingProgress === 0;

  return (
    <aside className="w-64 h-full bg-[#1A1A1A] text-stone-300 flex flex-col font-sans flex-shrink-0">
      {/* Top Section */}
      <div className="h-16 px-6 flex items-center gap-3 border-b border-stone-800">
        <div className="size-7 bg-stone-100 rounded-sm flex items-center justify-center text-stone-900 shadow-sm">
          <span className="font-bold font-mono text-xs tracking-tighter">O.</span>
        </div>
        <h2 className="text-stone-100 text-lg font-serif font-bold tracking-tight">O.N.E.</h2>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 flex flex-col gap-2">
        <NavLink
          ...
          className={({ isActive }) =>
            `flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
              isActive ? 'bg-stone-800 text-white' : 'hover:bg-stone-800/50 hover:text-white'
            }`
          }
        >
        ...
        {locked ? (
           <div className="flex items-center justify-between px-3 py-2 rounded-md text-sm font-medium text-stone-600 cursor-not-allowed">
